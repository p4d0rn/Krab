	...
	<view class="container">
		...
		<view class="content" v-if="Object.keys(order).length>0">
			<view class="order-box">
				...
				<view class="text-color-assist font-size-sm">感谢您对蟹堡王的支持，欢迎再次光临</view>
			</view>
				...
				<view class="order-info">
					...
					<list-cell padding="0 40rpx" line-right bgcolor="#f6f6f6">
						<view class="w-100 d-flex flex-column">
							<view class="d-flex align-items-center mt-40" v-for="(item, index) in order.dishes" :key="index">
								<view class="flex-fill d-flex flex-column">
									<view class="font-size-base mb-10">{{ item.dish.dishName }}</view>
								</view>
								<view class="flex-shrink-0 font-weight-bold ml-40">x{{ item.quantity }}</view>
								<view class="flex-shrink-0 font-weight-bold ml-40">￥{{ item.dish.dishPrice }}</view>
							</view>
							...
							<view class="d-flex justify-content-between align-items-center mt-40 pb-30 border-dashed">
								<view>是否使用优惠券</view>
								<view v-if="order.couponId == 0">无</view>
								<view v-if="order.couponId != 0">
					...
					<list-cell padding="30rpx 40rpx" last bgcolor="#f6f6f6" style="border-radius: 0 0 30rpx 30rpx;">
						<view class="w-100 d-flex flex-column font-size-extra-sm text-color-assist">
							...
							<view class="mb-10">下单时间：{{ order.orderTime }}</view>
							<view class="mb-10">取餐号：{{ order.pickupNo }}</view>
...
<script>
	...
	export default {
		...
		async onLoad({id}) {
			this.order = this.$store.state.orders.find(item => item.orderId == id)
			if(this.order.couponId != 0){
				const { data: res } = await uni.$http.get('/coupon?id=' + this.order.couponId)
