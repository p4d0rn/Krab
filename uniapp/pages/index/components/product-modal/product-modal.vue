	...
	<modal :show="visible" custom padding="0" width="90%" radius="18rpx">
		...
		<swiper :duration="1000" indicator-dots class="swiper" autoplay :interval="3000">
			<swiper-item :key="1" class="swiper-item">
				<image :src="productData.dishImg" class="w-100 h-100"></image>
			</swiper-item>
		</swiper>
		<scroll-view scroll-y class="content">
			<view class="wrapper">
				<view class="title">{{ productData.dishName }}</view>
				<view class="labels">
					<view class="label" v-for="(label, index) in productData.labels" :key="index" 
						:style="{color: label.labelColor, background: $util.hexToRgba(label.labelColor, 0.2)}">
						{{ label.labelName }}
					</view>
				</view>
		...
		<view class="bottom" :style="{height: !productData.is_single ? '250rpx' : '200rpx'}">
			<view class="d-flex align-items-center">
				<view class="price-and-materials">
					<view class="price">￥{{ productData.dishPrice }}</view>
